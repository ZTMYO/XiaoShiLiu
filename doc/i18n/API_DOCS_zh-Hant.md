...
| 參數 | 類型 | 必填 | 說明 |
|------|------|------|------|
| pinned | boolean | 是 | 是否置頂（true-置頂，false-取消置頂） |
...
搜尋框輸入時的候選補全，候選來自服務端進程內存索引，不查詢資料庫。
...
| 參數 | 類型 | 必填 | 說明 |
|------|------|------|------|
| q | string | 否 | 輸入內容，長度 1~100（超出截斷） |
| limit | int | 否 | 每組返回條數，預設5，上限10 |

**匹配規則**: 先按整串匹配；輸入達到 5 個字元後再按中文分詞拆出關鍵詞片段補充召回（如「啊啊美食啊啊」可召回「美食荒漠」）。候選先按命中片段長度降序，再按命中方式（詞首優先）與熱度降序排列。中文按包含匹配；純字母數字輸入額外按拼音全拼前綴、首字母前綴匹配（如 `shipin`、`sp` 均可命中「視頻」）。響應中每項的 `matched` 是候選文本裡被命中的原文片段，供前端高亮；拼音或小石榴號命中時無法定位原文，該欄位為空字串。
...
**邊界行為**: `q` 為空、索引尚未構建完成或內部異常時，三組均返回空數組，不返回 5xx。
